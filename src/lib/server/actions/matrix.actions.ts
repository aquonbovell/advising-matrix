...
export async function fetchDegree(studentId: string): Promise<Program> {
	...
	studentData.requirements = [...major, ...minor].map((requirement) => {
		const mappedRequirement = {
			...
			details: requirement.details.split(',').filter((d) => d.length > 0) as string[],
			courses: [],
...
export async function fetchDegreeExplore(
	majorId: string,
	...
	const major = await db
		.selectFrom('Majors')
		.innerJoin('MajorRequirement', 'Majors.id', 'MajorRequirement.majorId')
		.where('Majors.id', 'in', [majorId, minorId])
		.select([
			'Majors.name',
			'MajorRequirement.id',
			'MajorRequirement.credits',
	...
	const minor = await db
		.selectFrom('Minors')
		.innerJoin('MinorRequirement', 'Minors.id', 'MinorRequirement.minorId')
		.where('Minors.id', 'in', [majorId, minorId])
		.select([
			'Minors.name',
			'MinorRequirement.id',
			'MinorRequirement.credits',
...
export async function fetchStudentCourses(studentId: string) {
	...
	return studentCourses.map((course) => {
		return {
			...course,
			grade: (JSON.parse(course.grade) as string[]).map((grade) => grade as NonNullableGrade)
		};
	});
}
