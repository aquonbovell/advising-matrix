...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content>
		<DropdownMenu.Group>
			...
			<DropdownMenu.Item
				onclick={() => {
					...
					toast.success('Department ID copied to clipboard', { duration: 2000 });
				}}
			>
				Copy Department ID
			</DropdownMenu.Item>
		...
		<DropdownMenu.Item
			><Button.Root href={`/departments/${id}`} variant="ghost" class="m-0 h-fit p-0"
				>View Department</Button.Root
			></DropdownMenu.Item
		>
		{#if role === 'ADMIN'}
			<DropdownMenu.Item>
				<Button.Root href={`/departments/${id}/edit`} variant="ghost" class="m-0 h-fit p-0"
					>Edit Department
				</Button.Root>
			</DropdownMenu.Item>
		{/if}
