...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content>
		<DropdownMenu.Group>
			...
			<DropdownMenu.Item
				onclick={() => {
					...
					toast.info('Course ID copied to clipboard', { duration: 2000 });
				}}
			>
				Copy Course ID
			</DropdownMenu.Item>
		...
		<DropdownMenu.Item
			><Button.Root href={`/courses/${id}`} variant="ghost" class="m-0 h-fit p-0"
				>View Course</Button.Root
			></DropdownMenu.Item
		>
		{#if role === 'ADMIN'}
			<DropdownMenu.Item>
				<Button.Root href={`/courses/${id}/edit`} variant="ghost" class="m-0 h-fit p-0"
					>Edit Course
				</Button.Root>
			</DropdownMenu.Item>
		{/if}
