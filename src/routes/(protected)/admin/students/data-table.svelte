<script lang="ts">
	...
	$: users = trpc.admin.students.query({
		page: $index,
		size: $size,
		orderBy: $orderBy,
		search: $search.trim()
	});
	const paginatedData = writable<RouterOutputs['admin']['students']['students']>([]);

	const countStore = writable(0);

	$: {
		if ($users.isSuccess) {
			paginatedData.set($users.data.students);
							...
							{#each headerRow.cells as cell (cell.id)}
								<Subscribe attrs={cell.attrs()} let:attrs props={cell.props()} let:props>
									<Table.Head {...attrs} class="[&:has([role=checkbox])]:pl-3">
										...
										{:else if ['name', 'email', 'role'].includes(cell.id)}
