...
<form class="space-y-4">
	...
	<div class="grid grid-cols-2 gap-3 lg:grid-cols-12">
		...
		<Form.Field {form} name="credits" class="lg:col-span-4">
			<Form.Control let:attrs>
				...
				<Input {...attrs} bind:value={$formData.credits} type="number" readonly />
			</Form.Control>
			<Form.FieldErrors class="mt-2 text-sm" />
		</Form.Field>
		...
		<Form.Field {form} name="departmentId" class="col-span-full lg:col-span-4">
			<Form.Control let:attrs>
				...
				<Select.Root selected={selectedDepartment}>
					<Select.Trigger {...attrs} class="min-w-max">
						<Select.Value placeholder="Department not assigned" />
					</Select.Trigger>
				</Select.Root>
				<input hidden bind:value={$formData.departmentId} readonly name={attrs.name} />
			</Form.Control>
			<Form.FieldErrors />
		</Form.Field>
	</div>
	...
	<Form.Field {form} name="prerequisites">
		<Form.Control let:attrs>
			...
			<RadioGroup.Root disabled class="flex gap-4" bind:value={$formData.prerequisites.dataType}>
